...
Moghar and Hamiche (2020) [2], who trained LSTM models on two years of one-minute equity data across multiple sectors, demonstrated early on that LSTM-based predictors yield superior out-of-sample performance compared to classical benchmarks. Their rigorous backtesting revealed that LSTMs adapt dynamically to regime changes and internalize path-dependent features that static models overlook.

Building on this, Shen and Shafiq (2020) [5] integrated technical indicators such as MACD, RSI, and volatility bands into a multi-layer LSTM framework applied to two years of one-minute Chinese equity data. Their comprehensive feature engineering pipeline—combining feature extension (polarize, min–max scale, fluctuation percentage), recursive feature elimination, and PCA—yielded a bi-weekly trend prediction binary accuracy of 93.25\% (F1 = 0.9323), with a true positive rate of 96\% and true negative rate of 90\%, but this performance was reported on a very short (three-month) test period (Q1 2019) that was also used for validation during model selection, risking information leakage and optimistic results.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.50\textwidth]{diagram.png}
